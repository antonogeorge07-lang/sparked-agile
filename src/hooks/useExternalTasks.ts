...
import { maybeShowReconnectToast } from "@/lib/reconnectToast";
...
export function useExternalTasks(projectId: string | null) {
  ...
  const fetchJiraIssues = useCallback(async () => {
    ...
    try {
      ...
      if (maybeShowReconnectToast(data, 'jira') || data?.needsToken) {
        setJiraError(data?.error || 'Jira token not configured');
        return;
      }
  ...
  const fetchGithubIssues = useCallback(async () => {
    ...
    try {
      ...
      if (error) throw error;
      if (maybeShowReconnectToast(data, 'github') || data?.needsToken) {
        setGithubError(data?.error || 'GitHub token not configured');
        return;
      }
  ...
  const fetchGithubPRs = useCallback(async () => {
    ...
    try {
      ...
      if (error) throw error;
      if (maybeShowReconnectToast(data, 'github')) return;
  ...
  const createJiraIssue = useCallback(async (params: {
    ...
    if (error) throw error;
    if (maybeShowReconnectToast(data, 'jira')) return null;
  ...
  const updateJiraIssue = useCallback(async (issueKey: string, updates: Record<string, any>) => {
    ...
    if (error) throw error;
    if (maybeShowReconnectToast(data, 'jira')) return;
    if (data?.error) throw new Error(data.error);
  ...
  const createGithubIssue = useCallback(async (params: {
    ...
    if (error) throw error;
    if (maybeShowReconnectToast(data, 'github')) return null;
    if (data?.error) throw new Error(data.error);
