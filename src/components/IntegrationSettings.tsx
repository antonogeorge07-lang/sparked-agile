  ...
  const fetchTokenExpiry = async () => {
    try {
      ...
      const [githubResult, jiraResult, msResult] = await Promise.all([
        supabase.from('user_github_tokens').select('token_expires_at, oauth_provider').eq('user_id', user.id).maybeSingle(),
        supabase.from('user_jira_tokens').select('token_expires_at, oauth_provider').eq('user_id', user.id).maybeSingle(),
