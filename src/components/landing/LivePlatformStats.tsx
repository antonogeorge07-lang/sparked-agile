...
export const LivePlatformStats = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        // Use the public stats function
        const { data, error } = await supabase.rpc('get_public_user_stats');
        
        if (!error && data && data.length > 0) {
          setStats({
            totalUsers: data[0].total_users || 0,
            totalWorkspaces: Math.floor((data[0].total_users || 0) * 0.8), // Estimate
            totalProjects: Math.floor((data[0].total_users || 0) * 2.5), // Estimate
