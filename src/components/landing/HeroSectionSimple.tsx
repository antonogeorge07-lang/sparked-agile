...
export function HeroSectionSimple() {
  ...
  const trustPoints = [
    { key: "landing.hero.setup", icon: Zap },
    { key: "landing.hero.noCreditCard", icon: CheckCircle },
    { key: "landing.hero.freeForever", icon: Sparkles },
  ];

  return (
    <section
      className="relative min-h-[85vh] flex items-center py-20 md:py-28 px-4 overflow-hidden"
      ...
      <div className="container relative mx-auto max-w-6xl z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          <div className="space-y-8 text-center lg:text-left">
            ...
            >
              <Badge
                className="gap-2.5 px-5 py-2 bg-gradient-to-r from-primary/10 to-accent/10 text-foreground border-primary/20 backdrop-blur-sm cursor-default shadow-lg"
                variant="outline"
              >
            ...
            >
              ...
              <motion.span
                className="block mt-4 text-xl sm:text-2xl md:text-3xl font-medium text-muted-foreground"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
            ...
            {/* CTAs */}
            <motion.div
              className="relative inline-flex flex-col sm:flex-row gap-4 pt-4"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.6, delay: 0.4 }}
            ...
            {/* Trust indicators */}
            <motion.div
              className="flex flex-wrap justify-center lg:justify-start gap-6 pt-2"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              transition={{ duration: 0.6, delay: 0.6 }}
            >
              {trustPoints.map(({ key, icon: Icon }, index) => (
                <motion.div
                  key={key}
