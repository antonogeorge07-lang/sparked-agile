...
serve(async (req) => {
  ...
  try {
    ...
    // Verify HMAC-signed state and trust only fields from the verified payload
    let stateData: { userId: string; redirectUrl?: string; origin?: string };
    try {
      stateData = await verifyOAuthState<{ userId: string; redirectUrl?: string }>(state);
    } catch (e) {
      throw new Error(`Invalid state parameter: ${(e as Error).message}`);
    }
    if (!stateData.userId) throw new Error('Invalid OAuth state payload');
    ...
    const tokenResponse = await fetch('https://github.com/login/oauth/access_token', {
      ...
      body: JSON.stringify({
        ...
        code: code,
        redirect_uri: oauthCallbackUrl(safeOAuthOrigin(stateData.origin), 'github'),
      }),
    });
    ...
    const redirectUrl = safeRedirectPath(stateData.redirectUrl, '/integrations');
    if (url.searchParams.get('response') === 'json') {
      return new Response(
        JSON.stringify({ success: true, redirectUrl: redirectUrl + (redirectUrl.includes('?') ? '&' : '?') + 'github=success' }),
    ...
    return new Response(null, {
      ...
      headers: {
        ...corsHeaders,
        'Location': redirectUrl + (redirectUrl.includes('?') ? '&' : '?') + 'github=success',
