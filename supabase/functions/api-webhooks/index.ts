...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      const urlGuard = validateExternalUrl(url);
      if (!urlGuard.ok) {
        return new Response(
          JSON.stringify({ error: `Invalid webhook url: ${urlGuard.error}` }),
          { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );
      }
    ...
    if (req.method === "PUT" || req.method === "PATCH") {
      ...
      if (updates.url !== undefined) {
        const urlGuard = validateExternalUrl(updates.url);
        if (!urlGuard.ok) {
          return new Response(
            JSON.stringify({ error: `Invalid webhook url: ${urlGuard.error}` }),
            { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
