...
serve(async (req) => {
  ...
  try {
    ...
    if (!encryptionKey || !supabaseUrl || !supabaseServiceKey || !anonKey) {
      throw new Error('Required environment variables not configured');
    }

    const authHeader = req.headers.get('Authorization');
    const cronHeader = req.headers.get('x-cron-secret');
    const isServiceRole = !!authHeader && authHeader === `Bearer ${supabaseServiceKey}`;
    ...
    // Auth: service-role/cron bypass project check; otherwise require a JWT and verify project membership
