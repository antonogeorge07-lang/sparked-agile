...
serve(async (req) => {
  ...
  try {
    ...
    const callbackUrl = oauthCallbackUrl(safeOAuthOrigin(stateData.origin), 'jira');
    ...
    if (url.searchParams.get('response') === 'json') {
      return new Response(
        JSON.stringify({ success: true, redirectUrl: redirectUrl + (redirectUrl.includes('?') ? '&' : '?') + 'jira=success' }),
    ...
    return new Response(null, {
      ...
      headers: {
        ...corsHeaders,
        'Location': redirectUrl + (redirectUrl.includes('?') ? '&' : '?') + 'jira=success',
      },
    });
