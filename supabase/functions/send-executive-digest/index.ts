...
serve(async (req) => {
  ...
  try {
    ...
    // Scope epic ROI to this project only (prevents cross-tenant leak).
    // epic_roi_tracking → epics → value_streams → projects
    const { data: projectEpicIds } = await supabase
      .from("epics")
      .select("id, value_streams!inner(project_id)")
      .eq("value_streams.project_id", projectId);

    let epicRoi: Array<{ roi_percentage: number; epics: { title: string } | null }> = [];
    if (projectEpicIds && projectEpicIds.length > 0) {
      const ids = projectEpicIds.map((e: any) => e.id);
            ...
            messages: [
              {
                role: "system",
                content:
                  "You are an executive assistant providing brief, actionable project insights. Keep responses under 100 words.",
              },
              {
                role: "user",
    ...
    const safeAiInsights = escapeHtml(aiInsights);
    ...
    const emailHtml = `
      ...
      <head>
        <style>
          ...
          .wins { color: #059669; } .risks { color: #dc2626; } .insights { color: #2563eb; } .metrics { color: #7c3aed; }
      ...
      <body>
        <div class="container">
          <div class="header">
            ...
            <p style="margin: 10px 0 0 0; opacity: 0.9;">${safeProjectName} Weekly Summary</p>
          </div>
          <div class="content">
            ...
            <div class="section">
              ...
              ${completedItems && completedItems.length > 0
                ? `<ul>${completedItems.map((item: any) => `<li>${escapeHtml(item.title)}</li>`).join("")}</ul>`
                : '<p style="color: #64748b;">No completed items this week</p>'}
            </div>
            <div class="section">
              <div class="section-title risks">⚠️ Risks & Blockers</div>
              ${blockers && blockers.length > 0
                ? `<ul>${blockers.map((b: any) => `<li><strong>[${escapeHtml(b.priority)}]</strong> ${escapeHtml(b.title)}</li>`).join("")}</ul>`
    ...
    const emailResult = await resend.emails.send({
      ...
      to: [email],
      subject: `${isTest ? "[TEST] " : ""}📊 Weekly Executive Digest - ${safeProjectName}`,
      html: emailHtml,
    });
